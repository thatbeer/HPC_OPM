...
int main(){
  ...
  float u_avg[NX]; // array for averaged values with exclude boundary values
  ...
  /* Calculate vertically averaged distribution */
  for (int i=1; i<NX+1; i++){ // Exclude boundaries
    float sum = 0.0;
    for (int j=1; j<NY+1; j++){ // Exclude boundaries
      sum += u[i][j];
    }
    u_avg[i] = sum / (NY); // Average over the vertical domain
  }
  ...
  /* Write array of averaged u values to file */
  FILE *avgfile;
